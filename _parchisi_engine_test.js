...
ok('كلاسيك: قتل الخروج منح مكافأة 20 معلّقة (النرد المتبقي أولاً)', e.bonus && e.bonus.dist === 20 && e.phase === 'MOVING');
ok('كلاسيك: الخروج تمّ (pos 0)', pc(e, 0, 0).state === 'onboard' && pc(e, 0, 0).pos === 0);
e.applyMove(1, 3, 0);
ok('كلاسيك: بعد استهلاك النرد يظهر طور المكافأة 20', e.phase === 'BONUS' && e.bonus.dist === 20);
{
...
/* قتل الخروج مع نرد متبقٍ غير قابل للعب: المكافأة تُلعب قبل تسليم الدور */
e = mk(2, 'classic');
...
e.applyRoll([5, 3]);
ok('قتل الخروج + لا حركة للنرد المتبقي', pc(e, 1, 0).state === 'home' && e.phase !== 'MOVING');
ok('المكافأة نُفّذت قبل تسليم الدور (بيدق 1 → 24)', pc(e, 0, 1).pos === 24 && pc(e, 0, 0).pos === 0);
ok('انتهى الدور بعد المكافأة', e.phase === 'WAIT_ROLL' && e.current === 1);
...
ok('بقي نرد 2 للحركة', e.availableValues().length === 1 && e.availableValues()[0].v === 2);
