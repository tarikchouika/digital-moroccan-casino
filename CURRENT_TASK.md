...
## 11) v2.20 (2026-09-13) — الطاولة 🎲 + الضومنة 🁫 — لعبتان جديدتان بمعمارية المشاريع المستقلة

> الوكيل cat. مبني فوق دمج كامل لفرع arena حتى رأس sam `aff73c8` (v2.19). sam ينشر.

1. **دمج تزامني أولاً**: فروع sam حتى v2.19 (قاعدة البرية النهائية في رامي + روندا لاندسكيب + 2FA + واتساب + حذف tips القواعد) دُمجت كاملة قبل أي عمل — رامي 100/100 + قرعة 13/13 + BJMP 33/33 + v217 46/46 + v219 15/15 كلها خضراء بعد الدمج.
2. **مشروعان مستقلان بعقد المنصة** (نمط ronda-game): `backgammon-game/` (bg) و`dominoes-game/` (do) — محركان حتميان مدققان + ذكاء ثلاثي المستويات لكل منهما + واجهات بهوية مستقلة (bw-/dm-).
3. **إصلاحات جوهرية في التدقيق**: محفظة حقيقية (takeBet/giveWin بدل take/give الوهميين — كانت اللعبتان تلعبان بمحفظة 500 محلية متجاهلة رصيد اللاعب) + recordRound للتذاكر + ازدواج نقاط الطاولة (مباراة هدف 1 كانت تنتهي 6:4!) + RTL تلميحات الضومنة + مزامنة الكتم + ارتفاع المسرح 100%.
4. **الربط المركزي**: catalog 45 لعبة (featured بالمعرف الآن — الإدراج أزاح الفهارس) + GAME_IMG + initFor + ENG (getters كسولة) + cleanup في closeGamePage + 16 وسم script ووسمي link (v=bgdo1) + 103 مفاتيح ترجمة بأربع لغات + FULL_RULES بأربع لغات (بلا tips) + أيقونات وخلفيات بهوية المنصة + deploy script ينقل المشروعين.
5. **الغرف**: غير مفعلة هذه الجولة — اللعبتان محليتان (AI + hotseat)؛ بلا معالجات rmove. موثق.
6. **اختبارات**: `_bg_do_integration_test.js` **30/30** (فتح/مباريات/انسحاب/إعادة فتح/كتالوج/فرنسية/صفر أخطاء) + `_bg_do_gameplay_test.js` **18/18` (مباريات كاملة حقيقية حتى matchEnd بتسوية مالية دقيقة وتذاكر صحيحة) + كل الانحدارات خضراء.
7. **إصلاح جانبي للبنية التحتية للاختبارات**: `_rd_pw.js` يسقط تلقائياً لثنائي Playwright المحلي على aarch64 (sparticuz x86-64 لا يعمل هنا) + locale ar-MA للسياق (كشف اللغة التلقائي غيّر عناوين الزائر بلا تفضيل).

### لـ sam: نشر Pages (السكربت يحمل المشروعين تلقائياً) — لا تغيير خادم مطلوب هذه الجولة. التفاصيل الكاملة في GITHUB_SYNC.md قسم v2.20.

## 12) v2.21 (2026-09-13) — إعادة تسمية المنصة: DTSG — ألعاب الدكاء التقليدية الرقمية

> الوكيل cat. الاسم الجديد: **ألعاب الدكاء التقليدية الرقمية / DTSG — Digital Traditional Skills Games**. sam ينشر + يعيد تشغيل خادمه (مُصدر TOTP).

1. **الأصول (من حزمة هوية المالك)**: `assets/dtsg/` — اللوغو الشفاف + بانرات داكن/مضيء (الرئيسية تتبدل مع الثيم) + خلفيات الصفحات للوضعين + بلاطة زليج + عملة وجهين (الأسد/الشعار) + فافيكونات 16→512 + ico.
2. **لوحة الألوان**: --gold #d9b45c · --gold2 #f2d489 · --bg #0b1526 كحلي زليج · radiant #f7f1e2 عاجي — في css/01-variables.css وكل الـ rgba الذهبية في 14 ملف CSS.
3. **الاسم**: الصفحات التسع عناوين/meta + الشريط الجانبي + التذييل + الترجمات بالأربع لغات (ui.brand، intros، heroSub، legal.*) + رؤوس كل JS + رسائل الكونسول + مُصدر TOTP → DTSG.
4. **فلتر «كازينو»**: «الطاولة والورق / Tables & Cards» (المعرف الوظيفي cat:'casino' لم يتغير).
5. **داخل الألعاب**: العلامة المائية dtsg-logo-watermark في كل مسارح gFrame + بلياردو + عملة القلب وجهي الأسد والشعار.
6. **كاش**: dtsg1 على كل المعاد تسميته في index + الصفحات القانونية.
7. **التحقق**: 56/56 فحص هوية بالمتصفح + 5/5 أصول داخل الألعاب + انحدار كامل (رامي 100/100، قرعة 13/13، BJMP 33/33، BGDO 30/30، v217 46/46، v219 15/15). مسح نهائي: صفر علامة قديمة ظاهرة.

### لـ sam: نشر Pages (أصول dtsg في git) + إعادة تشغيل الخادم (issuer TOTP). التفاصيل في GITHUB_SYNC.md قسم v2.21.
