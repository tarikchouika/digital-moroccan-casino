...
## v2.22 — نطاق timer-layout (ضاما + الشطرنج)

طلب المالك: مؤقت الدور كان محشوراً تحت أيقونة اللاعب — الآن بجانبها (نمط روندا).

- شارة ذهبية `.dama-ptimer`/`.ch-ptimer` على أيقونة صاحب الدور فقط (تُخفى على الآخر)، تتبع اتجاه اللغة، حالة low ≤10ث حمراء نابضة.
- إصلاح توقيت أول-ثانية: الرسم الأول كان قبل إنشاء _turnTi فتظهر الشارة متأخرة ثانية — أُعيد الترتيب في اللعبتين.
- عناصر النص القديمة (#damaTimer/#chessTimer) مخفية بصرياً وباقية في DOM (توافق الاختبارات).
- كاش `?v=tl1` على dama.js/chess.js/10-dama.css/12-chess.css في index.html.
- جديد: `tests/_timer_seat_probe_test.js` **25/25** (بورتريه+لاندسكيه، اللعبتان، دورة كاملة، قلب الشطرنج، هندسة عقد روندا).
- انحدار: `_dama_fixes_test` 15/15 ✓ · `_chess_browser_test` = HEAD نفسه (4 فشائل سابقة موجودة قبل النطاق) · node --check ✓.
- التفاصيل الكاملة في GITHUB_SYNC.md قسم v2.22 timer-layout.

## v2.22 — نطاق ai-expert (توحيد مستوى الخبراء في كل الألعاب)

طلب المالك: بوت الخبير في كل الألعاب (ضاما-الشطرنج-الطاولة-الضومنة) بمستوى الخبير وأعلى قوة ممكنة.

### 1) الطاولة (backgammon-game/js/engine/bg-game.js) — إصلاحان جوهريان
- **خطأ حقيقي كان يعكس الخبير:** إشارة ردّ الخصم كانت مجموعةً بدل مطروحة (`ev + replyV` بدل `ev − replyV`) فكان يختار المواضع التي ردُّ الخصم عليها أقوى! مقاسة: خبير ضد متوسط **0-8** (خسارة كاملة سابقاً) → بعد الإصلاح **10-4** · متوسط ضد خبير 7-7 عكسها 8-4 · خبير ضد مبتدئ 12-12 و14-0.
- **متوسط النرد الـ21 موزوناً** (دبل 1/36 · سواه 2/36) بدل رمية عشوائية واحدة + **حتمية كاملة** (صفر Math.random في مسار الخبير) + **كتاب افتتاح** للرميات الست المؤكدة (3-1 يصنع نقطة الخمسة فورياً في 6ms) + تقييم أثرى (صفوف نقاط البيت تربيعياً + كفاءة إخراج في السباق) + ميزانية تفكير داخلية (أسوأ افتتاح دبل ~850ms).
- أسوأ خطوة للخبير الآن < 1ث واقعياً (كانت تتقافز فوق 4ث).

### 2) الضومنة (dominoes-game/js/engine/domino-game.js) — عدالة + قوة
- **إصلاح غش معلوماتي:** الخبير كان يقرأ **يد الخصم الحقيقية** لتقدير انسداده — الآن كل التقدير من «غير المرئي» فقط (يد الذكاء + السلسلة المكشوفة؛ يد الخصم والبنك معاً غير مرئيين).
- وعي احتمالي جديد: احتمال انسداد الخصم من حصة غير المرئي بلا رد × وزن قرب الإفراغ + مكافأة متابعة الطرف الذي فتحته + هامش الهدف.
- القياسات: خبير ضد متوسط **15-24/24** · خبير ضد مبتدئ 21-24/24 · متوسط ضد مبتدئ فاصل واضح · fuzz 60 مباراة بكل الأزواج: صفر أخطاء.

### 3) الشطرنج (js/games/chess.js) — الميزانية صارت حقيقية
- **الخطأ:** ميزانية البوت (6ث) كانت تُفحص بين تكرارات العمق فقط — تكرار عمق 9 واحد كان يتجاوزها بحرية (قيست خطوات 20-26ث!). الآن فحص المهلة **داخل البحث** (كل 64 عقدة + عند كل حركة جذر): أسوأ خطوة ≈ الميزانية (+نافذة GC واحدة).
- **نسخة بحث خفيفة `chessCloneSearch`**: البحث كان ينسخ log/captured/rep في كل عقدة (ملايين المخصصات) — البحث يقرأ الرقعة والدور والحقوق فقط. سلامة مولّد الحركات مؤكدة: **perft(3)=8902 · perft(4)=197281** (أرقام مرجعية) — والفروع الحقيقية تظل بنسخة كاملة (تكرار المواقع/الأسرى محفوظان في اللعب الفعلي).

### 4) ضاما (js/games/dama.js) — الميزانية صارت حقيقية
- نفس إصلاح المهلة الداخلية (فحص كل 64 عقدة داخل search بمهلة 92% من الميزانية): الخبير (عمق 16/5ث) كان يتجاوزها فعلياً (قيست شملات 10.7ث) → الآن أسوأ شملة ≈ الميزانية (740ms على ميزانية 800ms في الاختبار). خيار احتياطي مضمون عند ميزانية شبه صفرية (شملة عمق 1 فورية).

### 5) توحيد التسمية — «خبير» للمستوى الأعلى في كل مكان
- bg.level.2/dm.level.2 في translations.js + bg-i18n.js + domino-i18n.js (عربي ودارجة «خبير»، مطابقة نمط ضاما) + جداول payouts في game-rules.js (4 لغات × لعبتان) + أزرار bg-html/domino-html — كان «محترف» بينما ضاما/الشطرنج «خبير».

### 6) الاختبارات
- **جديد: `tests/_ai_expert_test.js`** — **26/26** (كلها خضراء): هيمنة المستويات بالمحركين (كل الأزواج، بذور متعددة) + صفر حركات غير قانونية + عدالة الضومنة (لا قراءة يد الخصم — فحص كود آلي) + حتمية الخبير + كتاب الافتتاح + ميزانيات الوقت الأربع (bg/do/chess/dama) + perft الشطرنج + التسمية الموحدة.
- **انحدارات خضراء**: قرعة روندا 13/13 · BJMP 33/33 · رامي آس 21/21 · رامي سامبل v19 100/100 · رامي خبير exit 0 · v219 15/15 · **BGDO تكامل 30/30 + لعب كامل 18/18** (بإصلاح الاختبار نفسه: تعيين المستوى 1 صراحة قبل البدء — كان يفترض الافتراضي ويفشل إذا غيّرت prefs المستوى).
- كل ملف معدل: node --check ✓.

## v2.26 (2026-09-15) — غرف الطاولة والضومنة للإنتاج (cat) + دمج v2.23–v2.25 من sam

> النطاق الرئيسي: تفعيل **الغرف الجماعية** للعبتَي الطاولة (bg) والضومنة (do) — مضيف+ضيف عبر البث الحي (rmove/SSE) وضد البوت مع التسوية المالية الخادمية. + دمج كامل لعمل sam v2.23–v2.25 (عشرية الأرصدة + SEO/dtsg.pages.dev + استئناف رامي + بطاقات غنية).

### 1) الغرف (النطاق الرئيسي)
- **`backgammon-game/js/ui/bg-room.js` (جديد 579 سطراً) + `dominoes-game/js/ui/domino-room.js` (جديد 551 سطراً)**: وضع الغرفة الكامل — المضيف سائق الحقيقة (يبني من seed) والضيف يستقبل init ويبني المطابق؛ بث roll/move/endTurn/pass/draw/nextround/rematch عبر rmove؛ بوت المقعد الفارغ يلعب دوره تلقائياً؛ إعادة المباراة مقابل البوت ببذرة عشوائية جديدة (إصلاح: كانت seed=1 ثابتة).
- **`js/core/rooms.js`**: bg/do في roomGameIds (ثنائيتان) + gameOpts (طول مباراة bg: 1/3/5 · هدف الضومنة 50-200 + قاعدة السحب كلاسيكي/Block) + applyGameOpts على كل العملاء قبل البدء.
- **`server.js`**: bg/do في ROOM_GAMES_ALLOWED + **[RoomGold] حارس `/api/sync`**: أثناء غرفة جارية (playing) رصيد الخادم هو مصدر الحقيقة — رفض gold الواجهة كان يطمس خصم الرهان عند مزامنة الإغلاق (رصيد قديم 100000 يُعاد فوق الخصم). + تتبعات settleRound خلف DM_TEST_MODE.
- **`js/core/rooms.js` `_refreshGold`**: سحب الرصيد الحقيقي من `/api/me` عند بدء الجولة لكل العملاء (المالك من الاستجابة + الضيوف من البث) — ST.gold المحلي يتزامن فورًا.
- **إصلاحات بوت الغرف**: تمرير no-moves للبوت في bg (كان يعلق عند roll بلا حركات) + تحرير busy عند نهاية الجولة في domino-app (كان يظل true عالقًا بعد آخر play فتتعطل طبقة الجولة أمام الأتمتة).

### 2) الدمج مع sam v2.23–v2.25
- عشرية الأرصدة (r2 منزلتان — server.js يشمل settleRound غرفنا) + dtsg.pages.dev/SEO كامل + استئناف رامي التلقائي + أهداف غرفة الرامي الديناميكية + بطاقات bg/do الغنية + أقسام/فلاتر فارغة تختفي + إزالة lazy من صور البطاقات.
- تعارضات حُلّت: index.html (كاش موحد dtsg3/dtsg4 فوق dtsg2 سام — حارس الكاش يكسر تلقائيًا) · server.js settleRound (r2 + تتبعاتنا) · الصفحات القانونية الثمان (dtsg2 سام) · GITHUB_SYNC (تسلسل الأقسام بلا تكرار).
- ميزات الطرفين محفوظة ومتحقق منها: حارس /api/sync + غرف bg/do (نحن) · r2 العشرية + auto-resume + فلاتر (سام).

### 3) الاختبارات بعد الدمج (كلها خضراء)
- **مسبار الغرف `tests/_bgdo_room_probe.js` (جديد): 32/32** — bg مضيف+ضيف (دخول غرفة/بث init/تزامن seed/إعدادات/أدوار حية بلا أخطاء) + bg ضد بوت (خصم 100000→99990 + matchEnd + تراكب + تسوية) + do مضيف+ضيف + do ضد بوت (نفسها).
- **مراقب قاعدة البيانات**: كل مستخدمي الاختبار خصموا الرهان صحيحًا وصفر انعكاس طوال نافذة المراقبة (15 دقيقة) — إصلاح /api/sync مؤكد قاطعًا.
- BGDO تكامل 30/30 · لعب كامل 18/18 · AI expert **26/26** (فحص سابق 25/26 كان ضغط جهاز — على جهاز هادئ كامل) · timer-seat 25/25 · قرعة روندا 13/13 · BJMP 33/33 · رامي v19 100/100 · رامي آس 21/21 · رامي خبير ✓ · dama fixes 15/15 · node --check على كل المعدل ✓.

### لـ sam: نشر Pages + إعادة تشغيل الخادم (لا تغيير schema). التفاصيل في GITHUB_SYNC.md قسم v2.26.

## v2.27 (2026-09-15) — الإصدار المدموج الكامل + ترسيخ النفخ + أمان + blindResult

- **الدمج**: خط cat v2.26 كاملاً فوق خط sam 02368ae (git apply --3way لفرق 490abc8→93830c4) — تعارض chess.js حُسم بإبقاء v19-Master (فوق AI-MAX) · index.html باعتماد كتل v2.26 · bg-game.js باعتماد وصف الخبير الجديد.
- **ضاما**: قاعدة النفخ بقرار المالك (إلزام قانوني + حرية حركة + نفخ المُلزَمة) — رسالة النفخ موصولة · رقيقة #damaStake مضافة · 15 فحصاً متعارضاً صُححت (كانت تفترض الإلزام البنيوي الملغى).
- **أمان**: كلمات البذر/الإنتاج ⇒ متغيرات بيئة · CORS الووركر += dtsg.pages.dev · **تدوير كلمات الإنتاج مطلوب من المالك**.
- **blindResult**: معالج action:'blind' في /api/rooms/move — جمع سري + بث dirs عند اكتمال الزوج + تنظيف عند المغادرة · اختبار 9/9.
