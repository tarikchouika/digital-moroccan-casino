...
const Fair = {
  sha256: SHA256,
  /* نتيجة جولة من بذرة (keno: {numbers:[20]} / crash: {crash_at}) */
  outcome(seed, gameId) {
  ...
  /* تحقق من جولة منتهية: البذرة + اللعبة + النتيجة المعلنة */
  verify(seed, gameId, outcome) {
    if (!seed || !outcome) return false;
    let derived = null;
    try { derived = this.outcome(seed, gameId); } catch (e) { return false; }
    return JSON.stringify(derived) === JSON.stringify(outcome);
  }
};
