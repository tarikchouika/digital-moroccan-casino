...
(async () => {
  ...
  const jobs = [
    ['bl-v11-snooker-fs.png', { width: 1280, height: 800 }, 'blsn', true],
    ['bl-v11-8ball-desk-min.png', { width: 1280, height: 800 }, 'bl8', false]
  ];
  for (const [name, vp, game, fs] of jobs) {
    ...
    if (fs) { await p.evaluate(() => enterAppFullscreen()); } else { await p.evaluate(() => exitAppFullscreen()); }
